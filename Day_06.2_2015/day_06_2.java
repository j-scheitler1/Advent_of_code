...
class day_06_2 {
    ...
    public static void main(String[] args) {
        ...
        for (String line : lines) {
            ...
            if (matcher.find()) {
                ...
                if (line.startsWith("turn on")) {
                    adjustGrid(startX, startY, endX, endY, 1);
                } else if (line.startsWith("turn off")) {
                    adjustGrid(startX, startY, endX, endY, -1);
                } else if (line.startsWith("toggle")) {
                    adjustGrid(startX, startY, endX, endY, 2);
                }
            } else {
        ...
        for (int brightness : grid.values()) {
    ...
    public static void adjustGrid(int s_x, int s_y, int e_x, int e_y, int delta) {
        for (int i = Math.min(s_x, e_x); i <= Math.max(s_x, e_x); i++) {
            for (int j = Math.min(s_y, e_y); j <= Math.max(s_y, e_y); j++) {
                Point p = new Point(i, j);
                int currentBrightness = grid.getOrDefault(p, 0);
                int newBrightness = Math.max(currentBrightness + delta, 0);
                grid.put(p, newBrightness);
            }
        }
    }
}
