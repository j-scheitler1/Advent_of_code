...
class day_06 {
    ...
    public static Map<Point, Boolean> grid = new HashMap<Point, Boolean>();
    ...
    public static void main(String[] args) {
        ...
        for (String line : lines) {
            ...
            if (matcher.find()) {
                ...
                if (line.startsWith("turn on")) {
                    turnOnGrid(startX, startY, endX, endY);
                } else if (line.startsWith("turn off")) {
                    turnOffGrid(startX, startY, endX, endY);
                } else if (line.startsWith("toggle")) {
                    toggleGrid(startX, startY, endX, endY);
                }
            } else {
        ...
        System.out.println("Size of the true values: " + trueCount);
    }

    public static void turnOnGrid(int s_x, int s_y, int e_x, int e_y) {
    ...
    public static void turnOffGrid(int s_x, int s_y, int e_x, int e_y) {
        for (int i = s_x; i <= e_x; i++) {
            for (int j = s_y; j <= e_y; j++) {
                Point p = new Point(i, j);
                grid.put(p, false);
    ...
    public static void toggleGrid(int s_x, int s_y, int e_x, int e_y) {
        for (int i = s_x; i <= e_x; i++) {
            for (int j = s_y; j <= e_y; j++) {
                Point p = new Point(i, j);
                if (grid.containsKey(p)) {
                    grid.put(p, !grid.get(p));
                } else {
                    grid.put(p, true);
                }
            }
